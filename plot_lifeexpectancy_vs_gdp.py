...
x = np.linspace(start=4, stop=9, num=100)
...
y = reg.coef_[0][0] * x + reg.intercept_[0]
plt.plot(np.exp(x), y, c='green', alpha=0.5)

# annotate Japan with country name, make point bigger
for row in df.index:
    if df.loc[row, 'country_name'] == 'Japan':
        plt.scatter(data=df.loc[row, :], x='GDP', y='value', c='black', marker='o', s=10, alpha=1)
        plt.text(x=df.loc[row, 'GDP'], y=df.loc[row, 'value'], s='Japan' + ' ' + str(df.loc[row, 'year']), fontsize=10)
...
plt.title('Life expectancy vs GDP for countries in 1980, 1990 and 2020')
# plt.xscale('log')
